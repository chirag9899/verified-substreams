    ...
    return <>
        ...
        <form onSubmit={async (e) => {
            ...
            try {
                ...
                let output: string;
                try {
                    const trade = Trade.fromBinary(response.value);
                    output = JSON.stringify(trade, (key, value) => {
                        if (key === "security") {
                            return "0x" + bufferToHex(trade.securityAddress);
                        }
                        if (key === "order_ref") {
                            return "0x" + bufferToHex(trade.orderRef);
                        }
                        if (key === "party") {
                            return "0x" + bufferToHex(trade.party);
                        }
                        if (key === "counterparty") {
                            return "0x" + bufferToHex(trade.counterParty);
                        }
                        if (key === "order_type") {
                            return "0x" + bufferToHex(trade.orderType);
                        }
                        if (key === "currency_address") {
                            return "0x" + bufferToHex(trade.currencyAddress);
                        }
                        if (key === "price") {
                            return "0x" + bigintToHex(trade.price);
                        }
                        if (key === "traded_amount") {
                            return "0x" + bigintToHex(trade.tradedAmount);
                        }
                        if (key === "execution_date") {
                            return "0x" + bigintToHex(trade.executionDate);
                        }
                        return value;
                    }, 6);
                } catch (e) {
